public class SchedulerSJF extends Scheduler {
	SchedulerSJF(String inputFilename, SharedVars sv) {
		super(inputFilename);	
		this.sv = sv;
		this.sv.schedulerName = "SJF"; 
		role = this.sv.role;
	}
		...
		if (running == null) {
			for (City city : ready) {
				if (city.task.name.equals(role)) {
					if (running == null) {
						running = city;
					} else if (city.task.duration < running.task.duration) {
						running = city;
					} else if (city.task.duration == running.task.duration && hasHigherPriority(city, running)) {
						running = city;
					}
				}
			}
